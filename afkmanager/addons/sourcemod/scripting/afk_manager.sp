...
{
    name = "AFK Manager",
    author = "Rothgar, modifed by PepperKick",
...
public Action Timer_CheckPlayer(Handle Timer, int client) // General AFK Timers
{
	...
	if(g_bEnabled) // Is the AFK Manager Enabled
	{
        ...
        // Disabling this to AFK kick during warmup
		if (false && g_bWaitRound) // Are we waiting for the round to start
		{
#if _DEBUG > 1
