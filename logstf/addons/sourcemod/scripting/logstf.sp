...
public Plugin:myinfo = {
	...
	author = "F2, Modifed by PepperKick",
...
new Handle:g_hCvarApikey, Handle:g_hCvarApiUrl, Handle:g_hCvarTitle, Handle:g_hCvarAutoUpload, Handle:g_hCvarMidGameUpload, Handle:g_hCvarMidGameNotice;
...
public OnPluginStart() {
	...
	g_hCvarApiUrl = CreateConVar("logstf_api_url", "http://logs.tf", "Your logs.tf API URL", FCVAR_PROTECTED);
...
UploadLog(bool:partial) {
	...
	GetConVarString(g_hCvarApiUrl, apiUrl, sizeof(apiUrl));

	// Format URL string and append '/upload'
	FormatEx(apiUrl, sizeof(apiUrl), "%s/upload", apiUrl);

	AnyHttpForm form = AnyHttp.CreatePost(apiUrl);
...
public bool:ParseLogsResponse(const char[] contents) {
	...
	if (FindJsonValue(resBuff, "success", success, sizeof(success)) && StrEqual(success, "true", false) && FindJsonValue(resBuff, "url", url, sizeof(url))) {
		...
		FormatEx(g_sLastLogURL, sizeof(g_sLastLogURL), "%s%s", apiUrl, url);
		if (!g_bIsPartialUpload) {
